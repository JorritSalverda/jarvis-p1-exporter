...
import (
	...
	"github.com/JorritSalverda/jarvis-p1-exporter/client/bigquery"
	"github.com/JorritSalverda/jarvis-p1-exporter/client/config"
	"github.com/JorritSalverda/jarvis-p1-exporter/client/p1"
	"github.com/JorritSalverda/jarvis-p1-exporter/client/state"
...
var (
	...
	measurementFileConfigMapName = kingpin.Flag("state-file-configmap-name", "Name of the configmap with state file.").Default("jarvis-p1-exporter").OverrideDefaultFromEnvar("MEASUREMENT_FILE_CONFIG_MAP_NAME").String()
)
...
func main() {
	...
	p1Client, err := p1.NewClient(ctx, *p1USBDevicePath)
	if err != nil {
		log.Fatal().Err(err).Msg("Failed creating p1 client")
	}
	...
	measurement, err := p1Client.GetMeasurement(ctx, config, lastMeasurement)
	if err != nil {
